import { revokeConsentByToken } from "@/lib/consent/revoke-consent";
...
function redirectToResult(
  ...
  const basePath = buildRevokePath(token);
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const result = await revokeConsentByToken(supabase, token, reason);
