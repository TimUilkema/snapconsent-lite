create or replace function app.cancel_recurring_profile_baseline_request(
  p_profile_id uuid,
  p_request_id uuid
)
returns table (
...
create or replace function public.cancel_recurring_profile_baseline_request(
  p_profile_id uuid,
  p_request_id uuid
)
returns table (
...
as $$
  select *
  from app.cancel_recurring_profile_baseline_request(
    p_profile_id,
    p_request_id
  );
$$;

create or replace function public.replace_recurring_profile_baseline_request(
  p_profile_id uuid,
  p_request_id uuid,
  p_new_request_id uuid,
  p_new_token_hash text,
...
as $$
  select *
  from app.replace_recurring_profile_baseline_request(
    p_profile_id,
    p_request_id,
    p_new_request_id,
    p_new_token_hash,
...
revoke all on function app.cancel_recurring_profile_baseline_request(uuid, uuid) from public;
revoke all on function app.replace_recurring_profile_baseline_request(uuid, uuid, uuid, text, timestamptz) from public;

grant execute on function public.cancel_recurring_profile_baseline_request(uuid, uuid) to authenticated;
grant execute on function public.replace_recurring_profile_baseline_request(uuid, uuid, uuid, text, timestamptz) to authenticated;
